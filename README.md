...
拉新的时候还要做一个处理就是B在注册登陆的时候(普通登录不行，只有注册登陆的才能知道你是新用户和老用户，在页面使用token来判断新老没有用，因为不存在注册登陆的动作，不能做拉新的人数的统计)需要与A进行关联，才能知道是A拉新的用户
```
...
#### 102. 使用不同项目中的组件，可以暴露install方法，再使用该组件的项目中调用install的方法传入vue来进行注册
