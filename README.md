...
#### 26. h5在ios中是可以获取到cookie的然后安卓有几率是获取不到，所以在强更的时候ios不会报登陆过期的问题，安卓就会报这个问题，所以安卓中需要判断进入路由的时候如果没有token我们就要自己手动去安卓中去取出来cookie，设置好token在进入到页面中。我们不需要再请求的时候来判断cookie存在不，我们可以前置，再进入路由中来判断然后再去cookie进行设置之后再进入到路由中
